  ...
  return (
    <>
      ...
      <GamePageShell eyebrow="NEWS & GUIDE" title="하이로 소식" tabs={TABS} activeId={activeId}>
        {loading ? (
          <p style={{ color: TEXT2, fontSize: 14 }}>게시물을 불러오는 중입니다...</p>
        ) : !shown.length ? (
          <p style={{ color: TEXT2, fontSize: 14 }}>게시물이 없습니다.</p>
        ) : (
          <>
            <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e5e7eb", overflow: "hidden" }}>
              {shown.map((p, idx) => (
                <Link key={p.id} to={`/blog/${p.slug}`}
                  style={{ display: "flex", alignItems: "center", gap: 20, padding: "15px 20px", borderBottom: idx < shown.length - 1 ? "1px solid #f3f4f6" : "none", textDecoration: "none", transition: "background .12s" }}
                  onMouseEnter={e => { e.currentTarget.style.background = "#eff6ff"; }}
                  onMouseLeave={e => { e.currentTarget.style.background = "transparent"; }}>
                  <span style={{ fontSize: 11, color: "#9ca3af", whiteSpace: "nowrap", minWidth: 88 }}>{fmt(p.publishedAt)}</span>
                  <span style={{ fontSize: 14, fontWeight: 500, color: TEXT, flex: 1 }}>{p.title}</span>
              ...
              <div style={{ display: "flex", justifyContent: "center", gap: 6, marginTop: 36 }}>
                {Array.from({ length: pages }, (_, i) => i + 1).map(n => (
                  <button key={n} onClick={() => setPage(n)}
                    style={{ width: 34, height: 34, borderRadius: 6, border: "1px solid", cursor: "pointer", borderColor: page === n ? BLUE : "#e5e7eb", background: page === n ? BLUE : "transparent", color: page === n ? "#fff" : TEXT2, fontSize: 12, fontWeight: 600 }}>
                    {n}
                  </button>
                ))}
              </div>
