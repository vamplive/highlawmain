...
const ABOUT_DEFAULTS = {
  ...
  directions: {
    ...
    subway: "2호선 역삼역 4번 출구에서 선릉역 방향으로 도보 1~2분 (약 100m 직진 후 1층에 스타벅스가 있는 역삼KR빌딩 15층입니다.)",
        ...
        <div className="container" style={{ maxWidth: 1000 }}>
          ...
          {activeTab === "greetings" && (
            <div className="tab-content-active" key="greetings" style={{ maxWidth: 760, margin: "0 auto" }}>
              <SectionHeading
                eyebrow={settings.greetings?.eyebrow || "Introduction"}
                title={settings.greetings?.title || "Message from Partners"}
              />
              <div style={{ color: "var(--text-secondary)", fontWeight: 300, fontSize: 15, lineHeight: 1.9, display: "flex", flexDirection: "column", gap: 24, marginBottom: 60, textAlign: "left" }}>
                {(settings.greetings?.content || "").split("\n\n").map((para, i) => (
                  <p key={i}>
                    {para.split("\n").map((line, j) => (
                      <span key={j}>
                        {line}
            ...
            <div className="tab-content-active" key="values">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 stagger" style={{ marginBottom: 40 }}>
                {(settings.values?.items || []).map((v, i) => {
                  const Icon = VALUES[i]?.icon || Scale;
                  return (
                    <SurfaceCard
                      key={i}
                      style={{ padding: "36px 28px", display: "flex", flexDirection: "column", height: "100%" }}
                    >
                      <div style={{ marginBottom: 16 }}><Icon size={30} strokeWidth={1.3} color="var(--accent-gold)" /></div>
          ...
          {activeTab === "directions" && (
            <div className="tab-content-active" key="directions" style={{ maxWidth: 900, margin: "0 auto" }}>
              <SectionHeading
                eyebrow={settings.directions?.eyebrow || "DIRECTIONS"}
                title={settings.directions?.title || "법무법인 하이로 서울 사무소"}
              />
              ...
              <SurfaceCard style={{ padding: 0, overflow: "hidden", marginBottom: 36, border: "1px solid var(--border-subtle)" }}>
                ...
                <div style={{ display: "flex", gap: 12, justifyContent: "center", padding: "20px 28px", borderBottom: "1px solid var(--border-subtle)", background: "#fbfbfc" }}>
                  {settings.directions?.naverMapUrl && (
                    <a 
                      href={settings.directions.naverMapUrl} 
                      target="_blank" 
                  ...
                  {settings.directions?.kakaoMapUrl && (
                    <a 
                      href={settings.directions.kakaoMapUrl} 
                      target="_blank" 
                      rel="noopener noreferrer" 
                      style={{ 
                        background: "#FEE500", 
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 16, borderLeft: "1px solid var(--border-subtle)", paddingLeft: "16px" }} className="md:border-l md:pl-8">
                      ...
                      <div>
                        ...
                        <p style={{ fontSize: 14, color: "var(--text-secondary)", fontWeight: 300, lineHeight: 1.6, display: "flex", flexDirection: "column", gap: 4 }}>
                          {settings.directions?.phone && <span>대표전화: <strong>{settings.directions.phone}</strong></span>}
                          {settings.directions?.fax && <span>팩스번호: {settings.directions.fax}</span>}
                          {settings.directions?.email && <span>이메일: {settings.directions.email}</span>}
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          {activeTab === "probono" && (
            <div className="tab-content-active" key="probono">
              <SectionHeading
                eyebrow={settings.probono?.eyebrow || "PRO BONO"}
                title={settings.probono?.title || "사회적 책임과 온기를 채우는 하이로의 공익 활동"}
              />
              {settings.probono?.description && (
          ...
          {activeTab === "history" && (
            <div className="tab-content-active" key="history" style={{ maxWidth: 760, margin: "0 auto" }}>
              <SectionHeading
                eyebrow={settings.history?.eyebrow || "HISTORY"}
                title={settings.history?.title || "어제를 돌아보며 내일을 준비하는 하이로의 발자취"}
              />
