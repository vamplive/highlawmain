...
export default function QnaHubPage() {
  ...
  useEffect(() => {
    ...
    const listUrl = `/inquiry/questions?${params}`;

    Promise.all([
      api.get("/inquiry/categories"),
      api.get(listUrl),
      !slug ? api.get("/inquiry/questions?limit=10&featured=true") : Promise.resolve({ data: [] }),
    ])
      .then(([t, q, f]) => {
        if (cancelled) return;
        setTree(t.data || []);
  ...
  function handleTabClick(tabSlug) {
    if (slug) {
      ...
      navigate(tabSlug === "all" ? "/inquiry" : qnaCategoryUrl(tabSlug));
    } else {
    ...
    <div ref={ref}>
      ...
      <section style={{ background: "#f7f8fa", borderBottom: "1px solid var(--border-subtle)" }}>
        <div className="container" style={{ maxWidth: 1080, padding: "16px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          ...
          <button
            onClick={() => navigate("/inquiry/ask", { state: { categorySlug: slug } })}
            style={{
...
function Breadcrumb({ path }) {
  return (
    <nav aria-label="경로" style={{ fontSize: 12, color: "var(--text-muted)" }}>
      ...
      <Link to="/inquiry" style={{ color: path.length > 0 ? "inherit" : "var(--text-primary)", textDecoration: "none", fontWeight: path.length > 0 ? 400 : 500 }}>
        Q&A
      </Link>
      {path.map((n, i) => (
        <span key={n.id}>
