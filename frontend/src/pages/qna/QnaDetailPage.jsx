 ...
 * /inquiry/question/:slug 경로. 답변이 없으면 "답변 준비 중" 표시.
 */
...
export default function QnaDetailPage() {
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      setLoading(true);
      setError(null);
      api.get(`/inquiry/questions/${encodeURIComponent(slug)}`)
        .then((res) => {
          if (cancelled) return;
          ...
          if (catSlug) {
            api.get(`/inquiry/questions?limit=4&categorySlug=${encodeURIComponent(catSlug)}`)
              .then((r) => {
                if (cancelled) return;
  ...
  async function handleVerifyPassword(e) {
    ...
    setPwError(null);
    try {
      const res = await api.post(`/inquiry/questions/${encodeURIComponent(slug)}/verify`, { password: pwInput });
      setQuestion(res.data);
      setPwInput("");
    } catch (err) {
  ...
  if (error || !question) {
    return <ErrorView error={error} onBack={() => navigate("/inquiry")} />;
  }
        ...
        <section style={{ padding: "80px 24px", textAlign: "center", minHeight: "50vh" }}>
          <div style={{ maxWidth: 400, margin: "0 auto" }}>
            ...
            <div style={{ marginTop: 32 }}>
              <Link to="/inquiry" style={{ fontSize: 13, color: "var(--text-muted)", textDecoration: "underline" }}>
                Q&A 목록으로 돌아가기
              </Link>
            </div>
          </div>
      ...
      <section style={{ background: "var(--bg-dark)", color: "#fff", padding: "56px 24px", textAlign: "center" }}>
        <div className="container" style={{ maxWidth: 720 }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 12, flexWrap: "wrap" }}>
            ...
            <Link to="/inquiry/ask" className="font-en" style={{
              display: "inline-block", padding: "14px 36px", fontSize: 13,
              letterSpacing: "0.15em", color: "#fff",
...
function Breadcrumb({ breadcrumb }) {
  return (
    <nav aria-label="경로" style={{ fontSize: 12, color: "var(--text-muted)" }}>
      ...
      <Link to="/inquiry" style={{ color: "inherit", textDecoration: "none" }}>Q&A</Link>
      {breadcrumb.map((b) => (
        <span key={b.id}>
          <span style={{ margin: "0 8px" }}>›</span>
